...
class HealthRecordApp:
    def __init__(self, root):
        ...
        label_input_frame = ttk.Frame(root, padding=(0, 10))
        label_input_frame.grid(row=0, column=1, sticky="nsew")
        ...
        for i, label_text in enumerate(labels):
            ...
            self.input_boxes[label_text] = ttk.Entry(label_input_frame, width=50)
            self.input_boxes[label_text].grid(row=i, column=1, sticky="ew", padx=5, pady=5)
            self.input_boxes[label_text].config(state="disabled")

        attachment_frame = ttk.Frame(root, padding=(0, 10))
        attachment_frame.grid(row=1, column=1, sticky="nsew")

        button_frame = ttk.Frame(root, padding=(10, 10, 10, 0))
        button_frame.grid(row=0, column=0, sticky="ns")
        self.record_counter_label = ttk.Label(
            button_frame, text="Record: 001 of 001", font=("Helvetica", 10, "bold")
        )
        self.record_counter_label.grid(row=10, column=0)
        ...
        record_text_frame = ttk.Frame(root, padding=(0, 10))
        record_text_frame.grid(row=0, column=2, rowspan=3, sticky="nsew")
        ttk.Label(
            record_text_frame,
            text="Medical Record",
            font=("Helvetica", 14, "bold")
        ).pack(anchor="center")
        self.record_details_text = tk.Text(
            record_text_frame, wrap="word", width=45, height=25
        )
        self.record_details_text.pack(expand=True, fill="both", padx=5, pady=5)
        self.record_details_text.config(state="disabled")

        self.uploaded_documents_list = tk.Listbox(
            attachment_frame, width=30, height=10
        )
        self.uploaded_documents_list.bind(
            '<<ListboxSelect>>', self.on_record_selection
        )
    ...
    def load_records(self):
        ...
        self.update_record_count()
        self.current_record_index = self.uploaded_documents_list.size() - 1
        self.load_record_details()
            ...
            with self.conn:
                ...
                if (record := cursor.fetchone()):
                    ...
                    for label, value in zip(labels, record[1:]):
                        self.record_details_text.insert(tk.END, f"{label}:", "bold")
                        self.record_details_text.insert(tk.END, f"{value}\n\n")

    def get_record_details_from_ui(self):
        return {label: input_box.get().strip() for label, input_box in self.input_boxes.items()}
